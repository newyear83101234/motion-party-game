...
### 8.1 審查 → 修改 → 再審查的節點規則（2026-04-19 數字氣球事件後新增）

每次召喚審查員後流程：
1. 收到「必修」清單 → 修改程式碼
2. **若修改涉及「影響遊戲手感的數值」**（速度、碰撞半徑、計時、難度曲線、聲音密度），**必須再召喚對應審查員審一輪**才能 deploy
3. 若只是文字 / 註解 / 配色微調 → 可跳過再審

**例**：費倫說「氣球出生位置太低」→ 我把 BALLOON_FLOAT_BASE 從 -0.7 改 -1.3 → 這是手感數值改動 → **應再召喚費倫確認新數值是否仍合理**（否則容易過頭，數字氣球事件就是這樣 ship 了過快的版本）。

### 8.2 LLM 審查員的根本限制（必須認知）

LLM agent 是基於程式碼推理，**無法評估**：
- 真實音檔試聽（音色 / 情緒 / 整體混音平衡）
- 真實肢體動作 vs 物件軌跡的物理交互
- 多種感官同時刺激的綜合疲勞感

審查員應該**主動 flag**「此項需真機驗收」，不要假裝能評估。
最終驗收必須由阿葉真機跑一輪 60 秒，沒有任何 LLM agent 可以替代。
